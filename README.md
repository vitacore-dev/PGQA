...
- импорт SQL из серверного лога (`log_statement`): вкладка **«📜 log_statement»** — разбор `postgresql-*.log` или csvlog, группировка по тому же fingerprint, что и workload/журнал, перенос в поле запроса для `EXPLAIN`; опционально **загрузка лога по SSH** с хоста профиля (белый список путей, SFTP / `tail -c`) и подстановка `log_directory` из БД;
